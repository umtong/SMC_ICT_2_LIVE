...
def test_ranking_uses_goal_proximity_and_supports_fresh_projects():
    evaluation = tomllib.loads((ROOT / "config/evaluation.toml").read_text(encoding="utf-8"))
...
def test_work_claim_registry_has_deduplication_fields():
    ...
    required = {"claim_id", "worker_id", "objective_fingerprint", "scope_fingerprint", "base_revision", "lease_until", "overlap_reason"}
