...
def main() -> None:
    ...
    manifest = json.loads((root / "bundle_manifest_v2.json").read_text())
    pieces = []
    for index in range(manifest["part_count"]):
        name = f"source_bundle_v2.tar.gz.b64.part{index:02d}"
    ...
    with tarfile.open(fileobj=io.BytesIO(tar_bytes), mode="r:") as archive:
        ...
        if names != set(manifest["files"]):
            raise RuntimeError(f"unexpected source bundle members: {sorted(names)}")
