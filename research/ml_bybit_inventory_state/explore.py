...
def features(x:pd.DataFrame)->pd.DataFrame:
    ...
    # Diagnostic labels only. Exact execution scripts replace this with the first 1m open strictly after the 500ms activation boundary.
...
def table(x:pd.DataFrame):
    base=x[(x['is_complete']) & x['oi_observed'] & x['ar_observed'] & x['premium_observed'] & x['oi_available_at_ms_ok'] & x['ar_available_at_ms_ok'] & x['premium_available_at_ms_ok']].copy()
    base=base[(base.price_shock_3.abs()>=2.0)&(base.volume_z>=0.5)&base.oi_shock_3.notna()]
    keep=np.ones(len(base),dtype=bool); last=-10**18
    for j,t in enumerate(base.start_time_ms.to_numpy()):
        if t-last<30*60*1000: keep[j]=False
        else: last=t
    base=base.iloc[np.flatnonzero(keep)].copy(); base['dir']=np.sign(base.ret_3).astype(int)
    base['oi_bucket']=pd.cut(base.oi_shock_3,[-np.inf,-1,-0.25,0.25,1,np.inf],labels=['<-1','-1:-.25','flat','+.25:1','>1'])
    for h in [6,12,24,48,96]: base[f'cont_{h}']=base['dir']*base[f'fwd_{h}']
    rows=[]
    for year in [2021,2022,2023]:
      for bucket,g in base[base.year==year].groupby('oi_bucket',observed=True):
        row={'symbol':base.symbol.iloc[0],'year':year,'oi_bucket':str(bucket),'n':len(g),'mean_oi':g.oi_shock_3.mean(),'mean_price':g.price_shock_3.abs().mean()}
        for h in [6,12,24,48,96]: row[f'cont_{h}bp']=g[f'cont_{h}'].mean()*1e4; row[f'win_{h}']=(g[f'cont_{h}']>0).mean()
        rows.append(row)
    return base,pd.DataFrame(rows)

if __name__=='__main__':
    btc=features(load_asset('BTC')); eth=features(load_asset('ETH')); btc,eth=add_cross(btc,eth)
    out=Path('/mnt/data/work/ml_inventory_state'); pd.concat([btc,eth],ignore_index=True).to_parquet(out/'features_2021_2023.parquet',index=False)
    ev=[];tabs=[]
    for x in [btc,eth]: e,t=table(x);ev.append(e);tabs.append(t)
    events=pd.concat(ev,ignore_index=True);tbl=pd.concat(tabs,ignore_index=True);events.to_parquet(out/'events_initial.parquet',index=False);tbl.to_csv(out/'initial_bucket_table.csv',index=False)
    print(tbl.to_string(index=False));print('events',events.groupby(['symbol','year']).size())
