...
ROOT = Path('/mnt/data')
WORK = ROOT / 'turnover_time_sponsored_core'
CACHE = WORK / 'cache'
OUT = WORK / 'result'
for p in (CACHE, OUT): p.mkdir(parents=True, exist_ok=True)
sys.path.insert(0, str(ROOT / 'smc_work' / 'repo'))
...
def build_packets(x:pd.DataFrame, symbol:str)->pd.DataFrame:
    ...
    # Complete-hour turnover; no imputation.
    ...
    # target at day start uses completed hour ending there: preceding hour index.
    ...
    for day,g in x.groupby('day',sort=True):
        ...
        for idx in idxs:
            ...
            if acc>=target:
                ...
                start=None; acc=0.0
        # discard incomplete end-of-day packet; packets never cross UTC day.
    p=pd.DataFrame(rec)
    if p.empty: return p
...
def generate_events(x:pd.DataFrame,p:pd.DataFrame,funds:pd.DataFrame)->list[Event]:
    ...
    # state loss packet close schedule by event direction resolved independently.
    for i,r in p.iterrows():
        ...
        # first later packet state loss and its executable minute.
        ...
        for j in range(i+1,len(p)):
            ...
            break
        # resolve minute path. State exit at its executable open, adverse stop before state; target not credited over state order.
        ...
        for k in range(entry_idx,n):
            ...
            # gap/market structural state exit ordering
...
def replay(events:list[Event],xmap:dict[str,pd.DataFrame],fmap:dict[str,pd.DataFrame],cost_bp:int,year:int,remove:set[str]|None=None)->tuple[pd.DataFrame,dict,pd.DataFrame]:
    ...
    # candidate ordering at same entry uses intensity, displacement, BTC
    cand=sorted(cand,key=lambda e:(e.entry_ms,-e.intensity_z,-e.displacement_atr,0 if e.symbol=='BTCUSDT' else 1))
    nav=10000.0; free_ms=int(pd.Timestamp(f'{year}-01-01',tz='UTC').timestamp()*1000); trades=[]
    for e in cand:
        if e.entry_ms<free_ms: continue
        ...
        if qty<=0: continue
        price_pu=e.side*(e.outcome_price-e.entry); pnl=qty*(price_pu+e.funding_per_unit-cost)
        ret=pnl/nav; nav_before=nav; nav=max(0.0,nav+pnl)
        completed=e.outcome_reason!='boundary_mark'
        trades.append(dict(event_key=e.event_key,symbol=e.symbol,side=e.side,entry_ms=e.entry_ms,exit_ms=e.outcome_end_ms,
            entry=e.entry,exit=e.outcome_price,stop=e.stop,target=e.target,intensity_z=e.intensity_z,displacement_atr=e.displacement_atr,
            reason=e.outcome_reason,funding_per_unit=e.funding_per_unit,quantity=qty,nav_before=nav_before,nav_after=nav,account_return=ret,completed=completed))
        free_ms=e.outcome_end_ms+1 if completed else 10**30
    t=pd.DataFrame(trades)
    # Build daily liquidation-value NAV from selected positions.
