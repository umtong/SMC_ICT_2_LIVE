...
/**
 * Evidence-gated YouTube caption/audio transport probe.
 *
 * The prior public-caption harvest was blocked from GitHub-hosted runners. This
 * probe tests independent InnerTube implementations and, crucially, a bounded
 * public audio range that can feed Korean ASR when native captions are absent.
 * Metadata alone never counts as recovery.
 */
...
async function buildClients() {
  ...
  for (const clientType of CLIENTS) {
    try {
      ...
      console.error(`session ${clientType} failed: ${record.message}`);
      clients.set(clientType, { __session_error: record });
    }
  }
  return clients;
}
...
async function main() {
  ...
  await writeFile(path.join(OUTPUT, 'probe.json'), `${JSON.stringify(result, null, 2)}\n`, 'utf8');
