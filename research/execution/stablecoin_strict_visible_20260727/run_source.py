...
class ExactBatchClient(BaseClient):
    """Exact block timestamps in bounded batches with exact REST fallback."""
    ...
    def _post_batch(self, block_numbers: Iterable[int]) -> dict[int, int]:
        ...
        for attempt in range(7):
            try:
                ...
                response = self.session.post(RPC_ENDPOINT, json=payload, timeout=(15, 120))
                ...
                if not isinstance(body, list):
                    raise source.BlockscoutError(
                        f"batch response must be a list, got {type(body)!r}"
                    )
                by_id = {
                    int(item["id"]): item
                    for item in body
                ...
                last = exc
                self.errors.append(f"{RPC_ENDPOINT}: {type(exc).__name__}: {exc}")
                if attempt + 1 < 7:
                    time.sleep(min(30.0, 1.5 * (2**attempt)))
        raise source.BlockscoutError(f"exact timestamp batch failed: {last!r}")
    ...
    def block_timestamp(self, block_number: int) -> int:
        ...
        try:
            self.block_timestamp_cache.update(
                self._post_batch(range(start, start + BATCH_SIZE))
            )
        except Exception:
...
def self_test() -> None:
    ...
    assert len(observed) == 1 and observed[0][0] == 92 and observed[0][-1] == 131
    ...
    print("STRICT_PINNED_SOURCE_SELF_TEST_PASS")
