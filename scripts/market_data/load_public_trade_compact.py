#!/usr/bin/env python3
"""Load sparse 500 ms Bybit trade shards and derive exact completed bars."""
...
def oci_reference(symbol: str, month: str) -> str:
    return f"{OCI_REPOSITORY}:{symbol.lower()}-{month}-v3"
...
def load_observed_500ms(shard: str | Path, *, verify_hash: bool = True) -> pd.DataFrame:
    ...
    if len(matches) != 1:
        raise KeyError(f"expected one 500ms_observed file in {root}")
...
def materialize_500ms(
    ...
    *,
    ...
    """Reconstruct an exact 500 ms grid for a bounded interval.

    Missing sparse rows become covered no-trade buckets when the source day is
    available. Pre-listing or explicitly missing source days remain unavailable.
    """
    root = Path(shard).expanduser().resolve()
    ...
    if not observed.empty:
        ...
        for column in (*PRICE_COLUMNS, *SIDE_FLOW_COLUMNS):
            grid[column] = np.nan
    ...
    if (grid["observed"] & ~grid["source_available"]).any():
        raise RuntimeError("sparse shard contains observations on an unavailable source day")
    for column in SIDE_FLOW_COLUMNS:
    ...
    ordered = [
        ...
        "volume", "turnover", "trade_count", *SIDE_FLOW_COLUMNS,
...
def derive_seconds(frame_500ms: pd.DataFrame, interval_seconds: int) -> pd.DataFrame:
    ...
    expected_source_rows = interval_seconds * 2
    ...
    if starts[0] % interval_ms or (len(starts) > 1 and not np.all(np.diff(starts) == 500)):
        raise ValueError("500 ms input must be exact, consecutive and aligned to the derived interval")
    if len(starts) % expected_source_rows:
    ...
    if not (out["source_rows_total"] == expected_source_rows).all():
        raise RuntimeError("derived interval contains an incomplete 500 ms group")
    unavailable = ~out["source_available"]
    out.loc[unavailable, [*PRICE_COLUMNS, "volume", "turnover", *SIDE_FLOW_COLUMNS]] = np.nan
...
def first_executable_trade_after(
    ...
    *,
    ...
    if not 0 <= offset <= 499:
        raise RuntimeError("observed 500 ms row has an invalid first trade offset")
    return {
