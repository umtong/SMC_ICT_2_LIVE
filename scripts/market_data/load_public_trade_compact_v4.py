#!/usr/bin/env python3
"""Load V4 bucket-index sparse 500 ms shards and derive completed bars."""
...
def oci_reference(symbol: str, month: str) -> str:
    return f"{OCI_REPOSITORY}:{symbol.lower()}-{month}-v4"
...
def load_manifest(shard: str | Path, *, verify_hash: bool = True) -> dict:
    ...
    if manifest.get("schema_version") != 4:
        raise RuntimeError("V4 compact loader requires schema_version 4")
    if verify_hash:
...
def load_observed_500ms(shard: str | Path, *, verify_hash: bool = True) -> pd.DataFrame:
    ...
    if len(matches) != 1:
        raise KeyError(f"expected one 500ms_observed_v4 file in {root}")
    ...
    starts = month_start_ms + packed["bucket_index"].astype("int64") * 500
    frame = pd.DataFrame({"start_time_ms": starts})
    for column in (*PRICE_COLUMNS, *SIDE_FLOW_COLUMNS, "trade_count", *OFFSET_COLUMNS):
...
def materialize_500ms(
    ...
    *,
    ...
    if (grid["observed"] & ~grid["source_available"]).any():
        raise RuntimeError("V4 shard contains observations on an unavailable day")
