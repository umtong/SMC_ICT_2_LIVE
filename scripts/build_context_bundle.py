...
def main() -> int:
    ...
    for rel in HOT:
        ...
        manifest.append({"source":rel,"file":dst.name,"sha256":hashlib.sha256(src.read_bytes()).hexdigest()})
    (ROOT/"dist/context-manifest.json").write_text(json.dumps(manifest,indent=2),encoding="utf-8")
