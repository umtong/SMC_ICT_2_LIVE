...
def _run_stablecoin_strict_v3_hook() -> None:
    trigger = ROOT / "research" / "execution" / "stablecoin_causal_visible_20260727" / "RUN_TRIGGER_20260727T0024KST.txt"
    helper = ROOT / "scripts" / "stablecoin_causal_validation_hook.py"
    if os.environ.get("GITHUB_ACTIONS") != "true":
        return
    if os.environ.get("GITHUB_WORKFLOW") != "Validate project harness":
        return
    if os.environ.get("SMC_STABLECOIN_STRICT_V3_HOOK_ACTIVE") == "1":
        return
    if not trigger.exists() or not helper.exists():
        return
    os.environ["SMC_STABLECOIN_STRICT_V3_HOOK_ACTIVE"] = "1"
    subprocess.run([sys.executable, str(helper)], cwd=ROOT, env=os.environ.copy(), check=True)


_run_stablecoin_strict_v3_hook()
