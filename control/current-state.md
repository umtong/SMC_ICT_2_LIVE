...
Its immutable verified implementation precommits `maximum_hold_bars=96`, initializes every surviving trade to close at the timeout open exactly 480 minutes after entry, and only replaces that default when an ATR stop or state condition fires. That is an elapsed-time liquidation and conflicts with the fixed project rule that exits must arise from strategy logic. The historical result remains in the Result Registry but is not ranking-eligible under the current contract.
