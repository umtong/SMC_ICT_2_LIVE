...
The fixed 2024 portfolio takeover that combined the now-ineligible dynamic state-exit with aligned continuation is closed before any 2024 strategy outcome is opened.

The Coinbase spot-flow ML route is closed as `SOURCE_UNAVAILABLE`: the sealed Coinbase and Bybit Tardis URLs returned 404 before market rows or model fitting. This is not alpha evidence.

The Bybit mark/index raid-acceptance ML route is retired. Its model AUC `0.617694` and Brier `0.255842` were worse than the causal structural-distance baseline, and its nominal positive account path contained one trade that disappeared under winner removal.
...
- four-asset direct after-cost utility regression with turnover-aware KEEP/SWITCH decisions;
- path-continuity structural first-passage ML;
- Aave on-chain liquidation forced-flow source gate and conditional ML screen.
...
Finish the direct-utility gap-boundary correction without imputation, consume the path-continuity result, and finish the Aave source gate. If these routes fail, switch to a materially new forced-flow or inventory-transfer source rather than extending Donchian or legacy completed-bar grids.
